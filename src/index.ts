...
app.post("/order", (req:any, res:any)=>{
    ...
    const quantity : number = req.body.userId;
...
app.get("/quote", (req : any, res : any) => {
    if (asks.length === 0) {
        return res.status(503).json({ error: "The quotes are not available currently." });
    }

    const initialPrice : number = asks[0]?.price;
    if (initialPrice === undefined) {
        return res.status(503).json({ error: "The quotes are not available currently." });
    }

    const currentAsks : Order[] = [...asks];
    if (currentAsks.length === 0) {
        return res.status(503).json({ error: "The quotes are not available currently." });
    }

    const currentPrice : number= currentAsks[0]?.price;
    if (currentPrice === undefined) {
        return res.status(503).json({ error: "The quotes are not available currently." });
    }

    const diff : number = Math.abs(initialPrice - currentPrice);

    if (diff < initialPrice / 10) {
        return res.json({ quote_value: initialPrice });
    } else {
        return res.status(400).json({ error: "Market volatility detected. The value of the quote is unavailable." });
    }
});
